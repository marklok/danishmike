...
function preprocessCitations(
    ...
    // Replace [N], [N, M, ...], and [law-N] markers with §idx§ tokens.
    // [law-N] refs map to annotation ref 1000+N (auto-generated by backend).
    return text.replace(/\[(law-\d+|\d+(?:,\s*\d+)*)\]/g, (full, refsStr) => {
        let refs: number[];
        if ((refsStr as string).startsWith("law-")) {
            const n = parseInt((refsStr as string).replace("law-", ""), 10);
            refs = [1000 + n];
            // During streaming, annotations haven't arrived yet — hide the raw [law-N] label.
            if (!annotations.some((a) => a.ref === 1000 + n)) return "";
        } else {
            refs = (refsStr as string)
                .split(",")
