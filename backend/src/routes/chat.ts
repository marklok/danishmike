...
chatRouter.post("/", requireAuth, async (req, res) => {
    ...
    if (lastUser?.content) {
        try {
            ...
            lawContextExtra = (lawBlock ?? "") + (lawWarning ?? "") || undefined;
        } catch (err) {
    ...
    try {
        ...
        try {
            ...
            // If Claude is overloaded and there's an OpenAI key available, fall back.
            if (isOverloadedError(primaryErr) && effectiveApiKeys.openai) {
                console.warn("[chat/stream] Claude overloaded — falling back to OpenAI", PLATFORM_FALLBACK_MODEL);
                result = await runLLMStream({
                    apiMessages,
                    docStore,
                    docIndex,
